...
public class OutGoodsThread extends Thread {
    private final Logger logOutGoods = Logger.getLogger(OutGoodsThread.class);
...
//                    queryCloseDropGoodsDoor();
    ...
    private void moveFloor1(){
        ...
        while (flag){
            ...
            Log.w("happy", "发送左柜移层指令");logOutGoods.info("发送左柜移层指令");
            ...
            if (rec != null && rec.length >= 5) {
                ...
                Log.w("happy", "左柜移层反馈："+ str1);logOutGoods.info("左柜移层反馈："+ str1);
            ...
            if(times == 5){
                ...
                if(packageCount2 <= 0 || Error2){
                    ...
                    intent.setAction("out_goods_end");
                    intent.putExtra("sysorderNo", current_transaction_order_number);
    ...
    private void queryMoveFloor1(){
        ...
        Log.w("happy", "发送左柜移层查询");logOutGoods.info("发送左柜移层查询");
        SystemClock.sleep(delay);
        byte[] rec = serialPort485.receiveData();
        if (rec != null && rec.length >= 5) {
            StringBuilder str1 = new StringBuilder();
            ...
            Log.w("happy", "左柜移层查询反馈："+ str1);logOutGoods.info("左柜移层查询反馈："+ str1);
            if(rec[0] == (byte)0xE2 && rec[1] == rec.length && rec[2] == 0x00 && rec[4] == (byte)0x0F && rec[rec.length-2] == (byte)0xF1 /*&& isVerify(rec)*/){
                if(rec[6] == (byte)0x79 && rec[3] == (byte)0xC0 && rec[7] == (byte)0x59){
                    if(rec[18] == (byte)0x02){
    ...
    private void moveFloor2(){
        ...
        int times = 0;
        while (flag){
            Log.w("happy", "发送右柜移层指令");logOutGoods.info("发送右柜移层指令");
            ...
            if (rec != null && rec.length >= 5) {
                ...
                Log.w("happy", "右柜移层反馈："+ str1);logOutGoods.info("右柜移层反馈："+ str1);
                if(rec[0] == (byte)0xE2 && rec[1] == rec.length && rec[2] == 0x00 && rec[4] == (byte)0x0F && rec[rec.length-2] == (byte)0xF1 /*&& isVerify(rec)*/){
    ...
    private void queryMoveFloor2(){
        ...
        Log.w("happy", "发送右柜移层查询");logOutGoods.info("发送右柜移层查询");
        SystemClock.sleep(delay);
        byte[] rec = serialPort485.receiveData();
        if (rec != null && rec.length >= 5) {
            StringBuilder str1 = new StringBuilder();
            ...
            Log.w("happy", "右柜移层查询反馈："+ str1);logOutGoods.info("右柜移层查询反馈："+ str1);
            if(rec[0] == (byte)0xE2 && rec[1] == rec.length && rec[2] == 0x00 && rec[4] == (byte)0x0F && rec[rec.length-2] == (byte)0xF1 /*&& isVerify(rec)*/){
                if(rec[6] == (byte)0x79 && rec[3] == (byte)0xC1 && rec[7] == (byte)0x59){
                    if(rec[18] == (byte)0x02){
                        if((rec[9]&0x7E) != (byte)0x00 || (rec[8]&0x08) != (byte)0x00){//暂时不检查（Bit0=0，未执行动作；Bit0=1，已执行动作）位,移层不检查Bit7=1，Y轴出货门定位开关故障，未使用定位开关，新加升降台上止点开关即时状态（0=开，1=合）不能触发上止点，触发为合
    ...
    private void pushGoods1(){
        ...
        while (flag){
            ...
            if (rec != null && rec.length >= 5) {
                ...
                Log.w("happy", "左柜推货反馈："+ str1);logOutGoods.info("左柜推货反馈："+ str1);
                if(rec[0] == (byte)0xE2 && rec[1] == rec.length && rec[2] == 0x00 && rec[4] == (byte)0x0F && rec[rec.length-2] == (byte)0xF1 /*&& isVerify(rec)*/){
    ...
    private void queryPushGoods1(){
        ...
        Log.w("happy", "发送左柜推货查询");logOutGoods.info("发送左柜推货查询");
        SystemClock.sleep(delay+10);
        byte[] rec = serialPort485.receiveData();
        if (rec != null && rec.length >= 5) {
            StringBuilder str1 = new StringBuilder();
            ...
            Log.w("happy", "左柜推货查询反馈："+ str1);logOutGoods.info("左柜推货查询反馈："+ str1);
            if(rec[0] == (byte)0xE2 && rec[1] == rec.length && rec[2] == 0x00 && rec[4] == (byte)0x0F && rec[rec.length-2] == (byte)0xF1 /*&& isVerify(rec)*/){
                if(rec[6] == (byte)0x70 && rec[3] == (byte)0x80 && rec[7] == (byte)0x50){
                    if(rec[16] == (byte)0x02){
    ...
    private void pushGoods2(){
        ...
        while (flag){
            ...
            if (rec != null && rec.length >= 5) {
                ...
                Log.w("happy", "右柜推货反馈："+ str1);logOutGoods.info("右柜推货反馈："+ str1);
                if(rec[0] == (byte)0xE2 && rec[1] == rec.length && rec[2] == 0x00 && rec[4] == (byte)0x0F && rec[rec.length-2] == (byte)0xF1 /*&& isVerify(rec)*/){
    ...
    private void queryPushGoods2(){
        ...
        Log.w("happy", "发送右柜推货查询");logOutGoods.info("发送右柜推货查询");
        SystemClock.sleep(delay);
        byte[] rec = serialPort485.receiveData();
        if (rec != null && rec.length >= 5) {
            StringBuilder str1 = new StringBuilder();
            ...
            Log.w("happy", "右柜推货查询反馈："+ str1);logOutGoods.info("右柜推货查询反馈："+ str1);
            if(rec[0] == (byte)0xE2 && rec[1] == rec.length && rec[2] == 0x00 && rec[4] == (byte)0x0F && rec[rec.length-2] == (byte)0xF1 /*&& isVerify(rec)*/){
                if(rec[6] == (byte)0x70 && rec[3] == (byte)0x81 && rec[7] == (byte)0x50){
                    if(rec[16] == (byte)0x02){
                        if((rec[9]&0x80) != (byte)0x00){//货道下货光栅故障
    ...
    private void moveHorizontal1(){
        ...
        while (flag){
            ...
            Log.w("happy", "发送左柜水平移位指令");logOutGoods.info("发送左柜水平移位指令");
            SystemClock.sleep(delay);
            byte[] rec = serialPort485.receiveData();
            if (rec != null && rec.length >= 5) {
                StringBuilder str1 = new StringBuilder();
                ...
                Log.w("happy", "左柜水平移位反馈："+ str1);logOutGoods.info("左柜水平移位反馈："+ str1);
                if(rec[0] == (byte)0xE2 && rec[1] == rec.length && rec[2] == 0x00 && rec[4] == (byte)0x0F && rec[rec.length-2] == (byte)0xF1 /*&& isVerify(rec)*/){
    ...
    private void queryMoveHorizontal1(){
        ...
        Log.w("happy", "发送左柜水平移位查询");logOutGoods.info("发送左柜水平移位查询");
        SystemClock.sleep(delay);
        byte[] rec = serialPort485.receiveData();
        if (rec != null && rec.length >= 5) {
            StringBuilder str1 = new StringBuilder();
            ...
            Log.w("happy", "左柜水平移位查询反馈："+ str1);logOutGoods.info("左柜水平移位查询反馈："+ str1);
            if(rec[0] == (byte)0xE2 && rec[1] == rec.length && rec[2] == 0x00 && rec[4] == (byte)0x0F && rec[rec.length-2] == (byte)0xF1 /*&& isVerify(rec)*/){
                if(rec[6] == (byte)0x78 && rec[3] == (byte)0xC0 && rec[7] == (byte)0x58){
                    if(rec[16] == (byte)0x02){
    ...
    private void moveHorizontal2(){
        ...
        while (flag){
            ...
            Log.w("happy", "发送右柜水平移位指令");logOutGoods.info("发送右柜水平移位指令");
            SystemClock.sleep(delay);
            byte[] rec = serialPort485.receiveData();
            if (rec != null && rec.length >= 5) {
                StringBuilder str1 = new StringBuilder();
                ...
                Log.w("happy", "右柜水平移位反馈："+ str1);logOutGoods.info("右柜水平移位反馈："+ str1);
                if(rec[0] == (byte)0xE2 && rec[1] == rec.length && rec[2] == 0x00 && rec[4] == (byte)0x0F && rec[rec.length-2] == (byte)0xF1 /*&& isVerify(rec)*/){
    ...
    private void queryMoveHorizontal2(){
        ...
        Log.w("happy", "发送右柜水平移位查询");logOutGoods.info("发送右柜水平移位查询");
        SystemClock.sleep(delay);
        byte[] rec = serialPort485.receiveData();
        if (rec != null && rec.length >= 5) {
            StringBuilder str1 = new StringBuilder();
            ...
            Log.w("happy", "右柜水平移位查询反馈："+ str1);logOutGoods.info("右柜水平移位查询反馈："+ str1);
            if(rec[0] == (byte)0xE2 && rec[1] == rec.length && rec[2] == 0x00 && rec[4] == (byte)0x0F && rec[rec.length-2] == (byte)0xF1 /*&& isVerify(rec)*/){
                if(rec[6] == (byte)0x78 && rec[3] == (byte)0xC1 && rec[7] == (byte)0x58){
                    if(rec[16] == (byte)0x02){
                        if((rec[9]&0x46) != (byte)0x00){//水平移动错位只判断停机故障字
    ...
    private void moveFloorOutRevise1(){
        ...
        while (flag){
            ...
            Log.w("happy", "发送左柜移层到出货口(校正)指令");logOutGoods.info("发送左柜移层到出货口(校正)指令");
            SystemClock.sleep(delay);
            byte[] rec = serialPort485.receiveData();
            if (rec != null && rec.length >= 5) {
                StringBuilder str1 = new StringBuilder();
                ...
                Log.w("happy", "左柜移层到出货口(校正)反馈："+ str1);logOutGoods.info("左柜移层到出货口(校正)反馈："+ str1);
                if(rec[0] == (byte)0xE2 && rec[1] == rec.length && rec[2] == 0x00 && rec[4] == (byte)0x0F && rec[rec.length-2] == (byte)0xF1 /*&& isVerify(rec)*/){
                    if(rec[6] == (byte)0x79 && rec[3] == (byte)0xC0 && rec[7] == (byte)0x59){
                        if(rec[18] == (byte)0x00 || rec[18] == (byte)0x01 || rec[18] == (byte)0x03){
                            flag = false;
    ...
    private void queryMoveFloorOutRevise1(){
        mMotorControl.query((byte)0x01,(byte)0xC0,rimZNum1);
        Log.w("happy", "发送左柜移层到出货口(校正)查询");logOutGoods.info("发送左柜移层到出货口(校正)查询");
        SystemClock.sleep(delay);
        byte[] rec = serialPort485.receiveData();
        if (rec != null && rec.length >= 5) {
            StringBuilder str1 = new StringBuilder();
            ...
            Log.w("happy", "左柜移层到出货口(校正)查询反馈："+ str1);logOutGoods.info("左柜移层到出货口(校正)查询反馈："+ str1);
            if(rec[0] == (byte)0xE2 && rec[1] == rec.length && rec[2] == 0x00 && rec[4] == (byte)0x0F && rec[rec.length-2] == (byte)0xF1 /*&& isVerify(rec)*/){
                if(rec[6] == (byte)0x79 && rec[3] == (byte)0xC0 && rec[7] == (byte)0x59){
                    if(rec[18] == (byte)0x02){
                        if((rec[9]&0x7E) != (byte)0x00 || (rec[8]&0x08) != (byte)0x00){//暂时不检查（Bit0=0，未执行动作；Bit0=1，已执行动作）位,移层不检查Bit7=1，Y轴出货门定位开关故障，未使用定位开关，新加升降台上止点开关即时状态（0=开，1=合）不能触发上止点，触发为合
    ...
    private void moveFloorOutRevise2(){
        ...
        while (flag){
            ...
            Log.w("happy", "发送右柜移层到出货口(校正)指令");logOutGoods.info("发送右柜移层到出货口(校正)指令");
            SystemClock.sleep(delay);
            byte[] rec = serialPort485.receiveData();
            if (rec != null && rec.length >= 5) {
                StringBuilder str1 = new StringBuilder();
                ...
                Log.w("happy", "右柜移层到出货口(校正)反馈："+ str1);logOutGoods.info("右柜移层到出货口(校正)反馈："+ str1);
                if(rec[0] == (byte)0xE2 && rec[1] == rec.length && rec[2] == 0x00 && rec[4] == (byte)0x0F && rec[rec.length-2] == (byte)0xF1 /*&& isVerify(rec)*/){
                    if(rec[6] == (byte)0x79 && rec[3] == (byte)0xC1 && rec[7] == (byte)0x59){
                        if(rec[18] == (byte)0x00 || rec[18] == (byte)0x01 || rec[18] == (byte)0x03){
                            flag = false;
    ...
    private void queryMoveFloorOutRevise2(){
        mMotorControl.query((byte)0x01,(byte)0xC1,rimZNum2);
        Log.w("happy", "发送右柜移层到出货口(校正)查询");logOutGoods.info("发送右柜移层到出货口(校正)查询");
        SystemClock.sleep(delay);
        byte[] rec = serialPort485.receiveData();
        if (rec != null && rec.length >= 5) {
            StringBuilder str1 = new StringBuilder();
            ...
            Log.w("happy", "右柜移层到出货口(校正)查询反馈："+ str1);logOutGoods.info("右柜移层到出货口(校正)查询反馈："+ str1);
            if(rec[0] == (byte)0xE2 && rec[1] == rec.length && rec[2] == 0x00 && rec[4] == (byte)0x0F && rec[rec.length-2] == (byte)0xF1 /*&& isVerify(rec)*/){
                if(rec[6] == (byte)0x79 && rec[3] == (byte)0xC1 && rec[7] == (byte)0x59){
                    if(rec[18] == (byte)0x02){
                        if((rec[9]&0x7E) != (byte)0x00|| (rec[8]&0x08) != (byte)0x00){//暂时不检查（Bit0=0，未执行动作；Bit0=1，已执行动作）位,移层不检查Bit7=1，Y轴出货门定位开关故障，未使用定位开关，新加升降台上止点开关即时状态（0=开，1=合）不能触发上止点，触发为合
    ...
    private void moveFloorOut1(){
        ...
        while (flag){
            ...
            Log.w("happy", "发送左柜移层到出货口指令");logOutGoods.info("发送左柜移层到出货口指令");
            SystemClock.sleep(delay);
            byte[] rec = serialPort485.receiveData();
            if (rec != null && rec.length >= 5) {
                StringBuilder str1 = new StringBuilder();
                ...
                Log.w("happy", "左柜移层到出货口反馈："+ str1);logOutGoods.info("左柜移层到出货口反馈："+ str1);
                if(rec[0] == (byte)0xE2 && rec[1] == rec.length && rec[2] == 0x00 && rec[4] == (byte)0x0F && rec[rec.length-2] == (byte)0xF1 /*&& isVerify(rec)*/){
                    if(rec[6] == (byte)0x79 && rec[3] == (byte)0xC0 && rec[7] == (byte)0x59){
                        if(rec[18] == (byte)0x00 || rec[18] == (byte)0x01 || rec[18] == (byte)0x03){
                            flag = false;
    ...
    private void queryMoveFloorOut1(){
        mMotorControl.query((byte)0x01,(byte)0xC0,rimZNum1);
        Log.w("happy", "发送左柜移层到出货口查询");logOutGoods.info("发送左柜移层到出货口查询");
        SystemClock.sleep(delay);
        byte[] rec = serialPort485.receiveData();
        if (rec != null && rec.length >= 5) {
            StringBuilder str1 = new StringBuilder();
            ...
            Log.w("happy", "左柜移层到出货口查询反馈："+ str1);logOutGoods.info("左柜移层到出货口查询反馈："+ str1);
            if(rec[0] == (byte)0xE2 && rec[1] == rec.length && rec[2] == 0x00 && rec[4] == (byte)0x0F && rec[rec.length-2] == (byte)0xF1 /*&& isVerify(rec)*/){
                if(rec[6] == (byte)0x79 && rec[3] == (byte)0xC0 && rec[7] == (byte)0x59){
                    if(rec[18] == (byte)0x02){
                        if((rec[9]&0x7E) != (byte)0x00 || (rec[8]&0x08) != (byte)0x00){//暂时不检查（Bit0=0，未执行动作；Bit0=1，已执行动作）位,移层不检查Bit7=1，Y轴出货门定位开关故障，未使用定位开关，新加升降台上止点开关即时状态（0=开，1=合）不能触发上止点，触发为合
    ...
    private void moveFloorOut2(){
        ...
        while (flag){
            ...
            Log.w("happy", "发送右柜移层到出货口指令");logOutGoods.info("发送右柜移层到出货口指令");
            SystemClock.sleep(delay);
            byte[] rec = serialPort485.receiveData();
            if (rec != null && rec.length >= 5) {
                StringBuilder str1 = new StringBuilder();
                ...
                Log.w("happy", "右柜移层到出货口反馈："+ str1);logOutGoods.info("右柜移层到出货口反馈："+ str1);
                if(rec[0] == (byte)0xE2 && rec[1] == rec.length && rec[2] == 0x00 && rec[4] == (byte)0x0F && rec[rec.length-2] == (byte)0xF1 /*&& isVerify(rec)*/){
                    if(rec[6] == (byte)0x79 && rec[3] == (byte)0xC1 && rec[7] == (byte)0x59){
                        if(rec[18] == (byte)0x00 || rec[18] == (byte)0x01 || rec[18] == (byte)0x03){
                            flag = false;
    ...
    private void queryMoveFloorOut2(){
        mMotorControl.query((byte)0x01,(byte)0xC1,rimZNum2);
        Log.w("happy", "发送右柜移层到出货口查询");logOutGoods.info("发送右柜移层到出货口查询");
        SystemClock.sleep(delay);
        byte[] rec = serialPort485.receiveData();
        if (rec != null && rec.length >= 5) {
            StringBuilder str1 = new StringBuilder();
            ...
            Log.w("happy", "右柜移层到出货口查询反馈："+ str1);logOutGoods.info("右柜移层到出货口查询反馈："+ str1);
            if(rec[0] == (byte)0xE2 && rec[1] == rec.length && rec[2] == 0x00 && rec[4] == (byte)0x0F && rec[rec.length-2] == (byte)0xF1 /*&& isVerify(rec)*/){
                if(rec[6] == (byte)0x79 && rec[3] == (byte)0xC1 && rec[7] == (byte)0x59){
                    if(rec[18] == (byte)0x02){
                        if((rec[9]&0x7E) != (byte)0x00 || (rec[8]&0x08) != (byte)0x00){//暂时不检查（Bit0=0，未执行动作；Bit0=1，已执行动作）位,移层不检查Bit7=1，Y轴出货门定位开关故障，未使用定位开关，新加升降台上止点开关即时状态（0=开，1=合）不能触发上止点，触发为合
    ...
    private void openOutGoodsDoor1(){
        ...
        while (flag){
            ...
            Log.w("happy", "发送左柜开出货门指令");logOutGoods.info("发送左柜开出货门指令");
            SystemClock.sleep(delay);
            byte[] rec = serialPort485.receiveData();
            if (rec != null && rec.length >= 5) {
                StringBuilder str1 = new StringBuilder();
                ...
                Log.w("happy", "左柜开出货门反馈："+ str1);logOutGoods.info("左柜开出货门反馈："+ str1);
                if(rec[0] == (byte)0xE2 && rec[1] == rec.length && rec[2] == 0x00 && rec[4] == (byte)0x0F && rec[rec.length-2] == (byte)0xF1 /*&& isVerify(rec)*/){
    ...
    private void queryOpenOutGoodsDoor1(){
        ...
        Log.w("happy", "发送左柜开出货门查询");logOutGoods.info("发送左柜开出货门查询");
        SystemClock.sleep(delay);
        byte[] rec = serialPort485.receiveData();
        if (rec != null && rec.length >= 5) {
            StringBuilder str1 = new StringBuilder();
            ...
            Log.w("happy", "左柜开出货门查询反馈："+ str1);logOutGoods.info("左柜开出货门查询反馈："+ str1);
            if(rec[0] == (byte)0xE2 && rec[1] == rec.length && rec[2] == 0x00 && rec[4] == (byte)0x0F && rec[rec.length-2] == (byte)0xF1 /*&& isVerify(rec)*/){
                if(rec[6] == (byte)0x6F && rec[3] == (byte)0xC0 && rec[7] == (byte)0x5A){
                    if(rec[16] == (byte)0x02){
    ...
    private void openOutGoodsDoor2(){
        ...
        while (flag){
            ...
            Log.w("happy", "发送右柜开出货门指令");logOutGoods.info("发送右柜开出货门指令");
            SystemClock.sleep(delay);
            byte[] rec = serialPort485.receiveData();
            if (rec != null && rec.length >= 5) {
                StringBuilder str1 = new StringBuilder();
                ...
                Log.w("happy", "右柜开出货门反馈："+ str1);logOutGoods.info("右柜开出货门反馈："+ str1);
                if(rec[0] == (byte)0xE2 && rec[1] == rec.length && rec[2] == 0x00 && rec[4] == (byte)0x0F && rec[rec.length-2] == (byte)0xF1 /*&& isVerify(rec)*/){
    ...
    private void queryOpenOutGoodsDoor2(){
        ...
        Log.w("happy", "发送右柜开出货门查询");logOutGoods.info("发送右柜开出货门查询");
        SystemClock.sleep(delay);
        byte[] rec = serialPort485.receiveData();
        if (rec != null && rec.length >= 5) {
            StringBuilder str1 = new StringBuilder();
            ...
            Log.w("happy", "右柜开出货门查询反馈："+ str1);logOutGoods.info("右柜开出货门查询反馈："+ str1);
            if(rec[0] == (byte)0xE2 && rec[1] == rec.length && rec[2] == 0x00 && rec[4] == (byte)0x0F && rec[rec.length-2] == (byte)0xF1 /*&& isVerify(rec)*/){
                if(rec[6] == (byte)0x6F && rec[3] == (byte)0xC1 && rec[7] == (byte)0x5A){
                    if(rec[16] == (byte)0x02){
                        if((rec[9]&0x3E) != (byte)0x00){//检查停机故障字
    ...
    private void moveHorizontalOut1(){
        ...
        while (flag){
            ...
            Log.w("happy", "发送左柜X轴出货指令");logOutGoods.info("发送左柜X轴出货指令");
            SystemClock.sleep(delay);
            byte[] rec = serialPort485.receiveData();
            if (rec != null && rec.length >= 5) {
                StringBuilder str1 = new StringBuilder();
                ...
                Log.w("happy", "左柜X轴出货反馈："+ str1);logOutGoods.info("左柜X轴出货反馈："+ str1);
                if(rec[0] == (byte)0xE2 && rec[1] == rec.length && rec[2] == 0x00 && rec[4] == (byte)0x0F && rec[rec.length-2] == (byte)0xF1 /*&& isVerify(rec)*/){
                    if(rec[6] == (byte)0x78 && rec[3] == (byte)0xC0 && rec[7] == (byte)0x58){
                        if(rec[16] == (byte)0x00 || rec[16] == (byte)0x01 || rec[16] == (byte)0x03){
                            flag = false;
    ...
    private void queryMoveHorizontalOut1(){
        mMotorControl.query((byte)0x02,(byte)0xC0,rimZNum1);
        Log.w("happy", "发送左柜X轴出货查询");logOutGoods.info("发送左柜X轴出货查询");
        SystemClock.sleep(delay);
        byte[] rec = serialPort485.receiveData();
        if (rec != null && rec.length >= 5) {
            StringBuilder str1 = new StringBuilder();
            ...
            Log.w("happy", "左柜X轴出货查询反馈："+ str1);logOutGoods.info("左柜X轴出货查询反馈："+ str1);
            if(rec[0] == (byte)0xE2 && rec[1] == rec.length && rec[2] == 0x00 && rec[4] == (byte)0x0F && rec[rec.length-2] == (byte)0xF1 /*&& isVerify(rec)*/){
                if(rec[6] == (byte)0x78 && rec[3] == (byte)0xC0 && rec[7] == (byte)0x58){
                    if(rec[16] == (byte)0x02){
    ...
    private void moveHorizontalOut2(){
        ...
        while (flag){
            ...
            Log.w("happy", "发送右柜X轴出货指令");logOutGoods.info("发送右柜X轴出货指令");
            SystemClock.sleep(delay);
            byte[] rec = serialPort485.receiveData();
            if (rec != null && rec.length >= 5) {
                StringBuilder str1 = new StringBuilder();
                ...
                Log.w("happy", "右柜X轴出货反馈："+ str1);logOutGoods.info("右柜X轴出货反馈："+ str1);
                if(rec[0] == (byte)0xE2 && rec[1] == rec.length && rec[2] == 0x00 && rec[4] == (byte)0x0F && rec[rec.length-2] == (byte)0xF1 /*&& isVerify(rec)*/){
                    if(rec[6] == (byte)0x78 && rec[3] == (byte)0xC1 && rec[7] == (byte)0x58){
                        if(rec[16] == (byte)0x00 || rec[16] == (byte)0x01 || rec[16] == (byte)0x03){
                            flag = false;
    ...
    private void queryMoveHorizontalOut2(){
        mMotorControl.query((byte)0x02,(byte)0xC1,rimZNum2);
        Log.w("happy", "发送右柜X轴出货查询");logOutGoods.info("发送右柜X轴出货查询");
        SystemClock.sleep(delay);
        byte[] rec = serialPort485.receiveData();
        if (rec != null && rec.length >= 5) {
            StringBuilder str1 = new StringBuilder();
            ...
            Log.w("happy", "右柜X轴出货查询反馈："+ str1);logOutGoods.info("右柜X轴出货查询反馈："+ str1);
            if(rec[0] == (byte)0xE2 && rec[1] == rec.length && rec[2] == 0x00 && rec[4] == (byte)0x0F && rec[rec.length-2] == (byte)0xF1 /*&& isVerify(rec)*/){
                if(rec[6] == (byte)0x78 && rec[3] == (byte)0xC1 && rec[7] == (byte)0x58){
                    if(rec[16] == (byte)0x02){
                        if((rec[9]&0x38) != (byte)0x00){//X轴出货故障点只做记录
    ...
    private void closeOutGoodsDoor1(){
        ...
        while (flag){
            mMotorControl.closeOutGoodsDoor(1,rimZNum1);
            Log.w("happy", "发送左柜关出货门指令");logOutGoods.info("发送左柜关出货门指令");
            SystemClock.sleep(delay);
            byte[] rec = serialPort485.receiveData();
            if (rec != null && rec.length >= 5) {
                StringBuilder str1 = new StringBuilder();
                ...
                Log.w("happy", "左柜关出货门反馈："+ str1);logOutGoods.info("左柜关出货门反馈："+ str1);
                if(rec[0] == (byte)0xE2 && rec[1] == rec.length && rec[2] == 0x00 && rec[4] == (byte)0x0F && rec[rec.length-2] == (byte)0xF1 /*&& isVerify(rec)*/){
    ...
    private void queryCloseOutGoodsDoor1(){
        ...
        Log.w("happy", "发送左柜关出货门查询");logOutGoods.info("发送左柜关出货门查询");
        SystemClock.sleep(delay);
        byte[] rec = serialPort485.receiveData();
        if (rec != null && rec.length >= 5) {
            StringBuilder str1 = new StringBuilder();
            ...
            Log.w("happy", "左柜关出货门查询反馈："+ str1);logOutGoods.info("左柜关出货门查询反馈："+ str1);
            if(rec[0] == (byte)0xE2 && rec[1] == rec.length && rec[2] == 0x00 && rec[4] == (byte)0x0F && rec[rec.length-2] == (byte)0xF1 /*&& isVerify(rec)*/){
                if(rec[6] == (byte)0x63 && rec[3] == (byte)0xC0 && rec[7] == (byte)0x5A){
                    if(rec[16] == (byte)0x02){
                        if((rec[9]&0x3E) != (byte)0x00){//检查停机故障字
    ...
    private void closeOutGoodsDoor2(){
        ...
        while (flag){
            mMotorControl.closeOutGoodsDoor(2,rimZNum2);
            Log.w("happy", "发送右柜关出货门指令");logOutGoods.info("发送右柜关出货门指令");
            SystemClock.sleep(delay);
            byte[] rec = serialPort485.receiveData();
            if (rec != null && rec.length >= 5) {
                StringBuilder str1 = new StringBuilder();
                ...
                Log.w("happy", "右柜关出货门反馈："+ str1);logOutGoods.info("右柜关出货门反馈："+ str1);
                if(rec[0] == (byte)0xE2 && rec[1] == rec.length && rec[2] == 0x00 && rec[4] == (byte)0x0F && rec[rec.length-2] == (byte)0xF1 /*&& isVerify(rec)*/){
    ...
    private void queryCloseOutGoodsDoor2(){
        ...
        Log.w("happy", "发送右柜关出货门查询");logOutGoods.info("发送右柜关出货门查询");
        SystemClock.sleep(delay);
        byte[] rec = serialPort485.receiveData();
        if (rec != null && rec.length >= 5) {
            StringBuilder str1 = new StringBuilder();
            ...
            Log.w("happy", "右柜关出货门查询反馈："+ str1);logOutGoods.info("右柜关出货门查询反馈："+ str1);
            if(rec[0] == (byte)0xE2 && rec[1] == rec.length && rec[2] == 0x00 && rec[4] == (byte)0x0F && rec[rec.length-2] == (byte)0xF1 /*&& isVerify(rec)*/){
                if(rec[6] == (byte)0x63 && rec[3] == (byte)0xC1 && rec[7] == (byte)0x5A){
                    if(rec[16] == (byte)0x02){
                        if((rec[9]&0x3E) != (byte)0x00){//检查停机故障字
    ...
    private void homing1(){
        ...
        while (flag){
            ...
            Log.w("happy", "发送左柜归位指令");logOutGoods.info("发送左柜归位指令");
            SystemClock.sleep(delay);
            byte[] rec = serialPort485.receiveData();
            if (rec != null && rec.length >= 5) {
                StringBuilder str1 = new StringBuilder();
                ...
                Log.w("happy", "左柜归位反馈："+ str1);logOutGoods.info("左柜归位反馈："+ str1);
                if(rec[0] == (byte)0xE2 && rec[1] == rec.length && rec[2] == 0x00 && rec[4] == (byte)0x0F && rec[rec.length-2] == (byte)0xF1 /*&& isVerify(rec)*/){
    ...
    private void queryHoming1(){
        ...
        Log.w("happy", "发送左柜归位查询");logOutGoods.info("发送左柜归位查询");
        SystemClock.sleep(delay);
        byte[] rec = serialPort485.receiveData();
        if (rec != null && rec.length >= 5) {
            StringBuilder str1 = new StringBuilder();
            ...
            Log.w("happy", "左柜归位查询反馈："+ str1);logOutGoods.info("左柜归位查询反馈："+ str1);
            if(rec[0] == (byte)0xE2 && rec[1] == rec.length && rec[2] == 0x00 && rec[4] == (byte)0x0F && rec[rec.length-2] == (byte)0xF1 /*&& isVerify(rec)*/){
                if(rec[6] == (byte)0x72 && rec[3] == (byte)0xC0 && rec[7] == (byte)0x59){
                    if(rec[18] == (byte)0x02){
                        if((rec[9]&0x7E) != (byte)0x00 || (rec[8]&0x08) != (byte)0x00){//暂时不检查（Bit0=0，未执行动作；Bit0=1，已执行动作）位,移层不检查Bit7=1，Y轴出货门定位开关故障，未使用定位开关，新加升降台上止点开关即时状态（0=开，1=合）不能触发上止点，触发为合
    ...
    private void homing2(){
        ...
        while (flag){
            ...
            Log.w("happy", "发送右柜归位指令");logOutGoods.info("发送右柜归位指令");
            SystemClock.sleep(delay);
            byte[] rec = serialPort485.receiveData();
            if (rec != null && rec.length >= 5) {
                StringBuilder str1 = new StringBuilder();
                ...
                Log.w("happy", "右柜归位反馈："+ str1);logOutGoods.info("右柜归位反馈："+ str1);
                if(rec[0] == (byte)0xE2 && rec[1] == rec.length && rec[2] == 0x00 && rec[4] == (byte)0x0F && rec[rec.length-2] == (byte)0xF1 /*&& isVerify(rec)*/){
    ...
    private void queryHoming2(){
        ...
        Log.w("happy", "发送右柜归位查询");logOutGoods.info("发送右柜归位查询");
        SystemClock.sleep(delay);
        byte[] rec = serialPort485.receiveData();
        if (rec != null && rec.length >= 5) {
            StringBuilder str1 = new StringBuilder();
            ...
            Log.w("happy", "右柜归位查询反馈："+ str1);logOutGoods.info("右柜归位查询反馈："+ str1);
            if(rec[0] == (byte)0xE2 && rec[1] == rec.length && rec[2] == 0x00 && rec[4] == (byte)0x0F && rec[rec.length-2] == (byte)0xF1 /*&& isVerify(rec)*/){
                if(rec[6] == (byte)0x72 && rec[3] == (byte)0xC1 && rec[7] == (byte)0x59){
                    if(rec[18] == (byte)0x02){
                        if((rec[9]&0x7E) != (byte)0x00 || (rec[8]&0x08) != (byte)0x00){//暂时不检查（Bit0=0，未执行动作；Bit0=1，已执行动作）位,移层不检查Bit7=1，Y轴出货门定位开关故障，未使用定位开关，新加升降台上止点开关即时状态（0=开，1=合）不能触发上止点，触发为合
    ...
    private void openGetGoodsDoor(){
        ...
        while (flag){
            ...
            Log.w("happy", "发送开取货门指令");logOutGoods.info("发送开取货门指令");
            SystemClock.sleep(delayGetDoor1);
            byte[] rec = serialPort485.receiveData();
            if (rec != null && rec.length >= 5) {
                StringBuilder str1 = new StringBuilder();
                ...
                Log.w("happy", "开取货门反馈："+ str1);logOutGoods.info("开取货门反馈："+ str1);
                if(rec[0] == (byte)0xE2 && rec[1] == rec.length && rec[2] == 0x00 && rec[4] == (byte)0x0F && rec[rec.length-2] == (byte)0xF1 /*&& isVerify(rec)*/){
    ...
    private void queryOpenGetGoodsDoor(){
        mMotorControl.query((byte)0x07,(byte)0xE0,midZNum);
        Log.w("happy", "发送开取货门查询");logOutGoods.info("发送开取货门查询");
        SystemClock.sleep(delayGetDoor1);
        byte[] rec = serialPort485.receiveData();
        if (rec != null && rec.length >= 5) {
            StringBuilder str1 = new StringBuilder();
            ...
            Log.w("happy", "开取货门查询反馈："+ str1);logOutGoods.info("开取货门查询反馈："+ str1);
            if(rec[0] == (byte)0xE2 && rec[1] == rec.length && rec[2] == 0x00 && rec[4] == (byte)0x0F && rec[rec.length-2] == (byte)0xF1 /*&& isVerify(rec)*/){
                if(rec[6] == (byte)0x64 && rec[3] == (byte)0xE0 && rec[7] == (byte)0x4D){
                    if(rec[16] == (byte)0x02){
    ...
    private void closeGetGoodsDoor(){
        ...
        while (flag){
            ...
            Log.w("happy", "发送关取货门指令");logOutGoods.info("发送关取货门指令");
            SystemClock.sleep(delayGetDoor1);
            byte[] rec = serialPort485.receiveData();
            if (rec != null && rec.length >= 5) {
                StringBuilder str1 = new StringBuilder();
                ...
                Log.w("happy", "关取货门反馈："+ str1);logOutGoods.info("关取货门反馈："+ str1);
                if(rec[0] == (byte)0xE2 && rec[1] == rec.length && rec[2] == 0x00 && rec[4] == (byte)0x0F && rec[rec.length-2] == (byte)0xF1 /*&& isVerify(rec)*/){
    ...
    private void queryCloseGetGoodsDoor(){
        ...
        Log.w("happy", "发送关取货门查询");logOutGoods.info("发送关取货门查询");
        SystemClock.sleep(delayGetDoor1);
        byte[] rec = serialPort485.receiveData();
        if (rec != null && rec.length >= 5) {
            StringBuilder str1 = new StringBuilder();
            ...
            Log.w("happy", "关取货门查询反馈："+ str1);logOutGoods.info("关取货门查询反馈："+ str1);
            if(rec[0] == (byte)0xE2 && rec[1] == rec.length && rec[2] == 0x00 && rec[4] == (byte)0x0F && rec[rec.length-2] == (byte)0xF1 /*&& isVerify(rec)*/){
                if(rec[6] == (byte)0x75 && rec[3] == (byte)0xE0 && rec[7] == (byte)0x4D){
                    if(rec[16] == (byte)0x02){
                        if((rec[8]&0x04) == (byte)0x04) {
                            ...
                            if((rec[9]&0x3E) != (byte)0x00 || (rec[8]&0x08) != (byte)0x00){//检查停机故障字
                                ...
                                context.sendBroadcast(intent);
                                Log.w("happy", "本次交易完毕");logOutGoods.info("本次交易完毕");
                                VMMainThreadFlag = true;
                                mQuery1Flag = true;
                                mQuery2Flag = true;
                                mQuery0Flag = true;
